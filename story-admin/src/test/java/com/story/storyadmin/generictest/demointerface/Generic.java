package com.story.storyadmin.generictest.demointerface;
...
//        }
     ...
     * @param <T>
     */
    //public static <T extends Object> void showKeyValue3(Generic<?> obj){ //这两种写法都可以
    public static <T> void showKeyValue3(Generic<?> obj){
        log.info("泛型测试,key is " + obj.getKey());
    }


    /**
     * 针对有返回值的情况，在泛型方法中添加上下边界限制的时候，必须在权限声明与返回值之间的<T>上添加上下边界，即在泛型声明的时候添加
     * public <T> T showKeyValue4(Generic<T extends Number> obj)，编译器会报错："Unexpected bound"
     * 方法有参数和返回值
     * @param obj
     * @param <T>
     */
    public static  <T extends Number> void showKeyValue4(Generic<T> obj){
        log.info("泛型测试,key is " + obj.getKey());
    }
    ...
    public static void f4(){
        ...
        showKeyValue2(gInteger);   //编译通过

        Generic<String> gString= new Generic<String>("789");
        showKeyValue2(gString);   //编译通过
    ...
    public static void f5(){
        ...
        showKeyValue3(gInteger);   //编译通过

        Generic<String> gString= new Generic<String>("789");
        showKeyValue3(gString);   //编译通过

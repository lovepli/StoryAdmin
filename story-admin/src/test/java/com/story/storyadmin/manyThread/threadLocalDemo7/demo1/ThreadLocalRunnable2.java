 ...
 */
public class ThreadLocalRunnable2 implements Runnable {

    //ThreadLocal<StudentShareData2> studenThreadLocal = new ThreadLocal<StudentShareData2>();

    // 构造器注入依赖
    StudentShareData2 studentShareData;

    public ThreadLocalRunnable2(StudentShareData2 studentShareData){
    ...
    public void run() { // 执行线程方法，既线程需要做的事
       ...
       // StudentShareData2 student = StudentShareData2.getStudent(); //通过这个方法，为每个线程都独立的 new 一个 student 对象，每个线程的的 student 对象都可以设置不同的值
        StudentShareData2 student = StudentShareData2.getUniqueInstance();
        student.setAge(age);
    ...
    //private StudentShareData2 getStudent() {
    //    StudentShareData2 student = studenThreadLocal.get();
    //    if (null == student) {
    //        student = studentShareData.getUniqueInstance();

...
import com.story.storyadmin.utils.TreeUtil;
...
public class TreeUtil3 {
    ...
    public static List<MenuDemo> addRootNode(String rootName, Integer rootId, List<MenuDemo> children) {
        MenuDemo root = new MenuDemo();
        root.setMenuId(rootId);
        root.setMenuName(rootName);
        root.setChildren(children);
        List<MenuDemo> rootList = new ArrayList<>();
        rootList.add(root);
        return rootList;
    }
    /**
    ...
    public static void main(String[] args) {
        ...
        List<MenuDemo> menuList = new ArrayList<>();
        menuList.add(new MenuDemo(1, null, "节点1"));
        menuList.add(new MenuDemo(2, null, "节点2"));
        menuList.add(new MenuDemo(3, 1, "节点1.1"));
        menuList.add(new MenuDemo(4, 1, "节点1.2"));
        menuList.add(new MenuDemo(5, 3, "节点1.1.1"));
        //菜单集合为：
        ...
        //1、集合转为菜单树结构
        Collection<MenuDemo> menus = TreeUtil.toTree(menuList, "menuId", "parentId", "children", MenuDemo.class);
        //转换的菜单树为:
        ...
        //2、为菜单树添加根节点
        List<MenuDemo>  menu2list =addRootNode("导航目录", 0, (List<MenuDemo>) menus);
        //"有根节点的菜单树:"
        System.out.println(JSONUtil.toJsonStr(menu2list));

        //3、获取菜单树的所有子节点
        //获取树形结构中的所有叶子节点，前提是必须已经是树结构
        List<MenuDemo> menuLeafNode = TreeUtil.getAllLeafNode(menu2list);
        //"获取到的菜单树的所有自节点为："

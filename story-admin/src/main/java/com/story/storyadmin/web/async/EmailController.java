  ...
  public Result asyncCall() {
    ...
    try {
      ...
      EmailController emailController = (EmailController) applicationContext.getBean(EmailController.class);
       // EmailController emailController = ApplicationContextUtil.getBean(EmailController.class); // 或者使用的封装方法获取
      emailController.testAsyncTask();
        result = new Result(true, "异步方法调用成功！", null , ResultEnum.TOKEN_CHECK_SUCCESS.getCode());
    } catch (Exception e) {
        result = new Result(false, "异步方法调用失败！", null , ResultEnum.API_REQUEST_EXCEPTION.getCode());
      log.error("error!", e);
    }
    return result;
  }
     ...
     */
    @RequestMapping(value = "/email/asyncCall2", method = GET)

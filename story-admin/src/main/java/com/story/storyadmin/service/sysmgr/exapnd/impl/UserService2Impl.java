...
import com.story.storyadmin.mapper.sysmgr.expand.UserMapper2;
import com.story.storyadmin.service.sysmgr.LoginLogService;
import com.story.storyadmin.service.sysmgr.exapnd.UserService2;
 ...
 */
@Service
public class UserService2Impl extends ServiceImpl<UserMapper2, User2> implements UserService2 {

    private static final Logger logger = LoggerFactory.getLogger(UserService2Impl.class);
     ...
     */
    @Override
    public User2 findUserByAccount(String account) {
        ...
        // TODO 为什么查询集合，有重名的人？
        List<User2> userList = baseMapper.selectList(wrapper);
        // TODO 返回集合第一个元素？
    ...
    public Result login(UserVo user, HttpServletResponse response) {
        ...
        User2 userBean = this.findUserByAccount(user.getUsername());
...
//        }
     ...
     */
    @Override
    public Result persist(User2 user) {
        Result result = null;
        //当前系统日期
        Date currentDate = Date.from(Instant.now());
        //添加
        if (user.getId() == null) {
            User2 existUser = this.findUserByAccount(user.getAccount());
            if (existUser != null) {
                //账号已存在
...
//                user.setStatus(UserStatusEnum.NORMAL.code());
            ...
            User2 userBean = baseMapper.selectById(user.getId());
    ...
    public Result editPassWord(UserPassword userPassword) {
        ...
        if (!StringUtils.isEmpty(userPassword.getPassword()) && !StringUtils.isEmpty(userPassword.getNewPassword())) {
            ...
            //直接从应用上下文获取当前登录用户信息
            User2 user = this.findUserByAccount(UserContext.getCurrentUser().getAccount());
    ...
    @Override
    public User2 selectUserById(Long id) {
        return baseMapper.selectById(id);
    }

    @Override
    public List<User2> selectUserNameList() {

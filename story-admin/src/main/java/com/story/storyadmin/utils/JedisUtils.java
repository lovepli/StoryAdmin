...
/**
 * Redis 访问操作工具类：JedisUtils类是封装 RedisTemplate<String, Object> 对象，对进行缓存的增删改查操作
 *
 * redis基本数据类型
 * 1、字符串：（缓存）
 * key：value value 可以是对象转换成的 JSON 字符串，也可以是对象序列化后的二进制字符串
...
public class JedisUtils {
    ...
    /** 导入RedisTemplate或者StringRedisTemplate */
    //这个redisTemplate和下面的 @Resource(name = "redisTemplate")中的redisTemplate是同一个对象吗？？
  @Autowired
  private RedisTemplate<String, Object> redisTemplate;

  //@Resource(name = "redisTemplate")注解中对redisTemplate是从RedisCacheConfig中加载进来的自定义 RedisTemplate<String, Object>模版类
  //可以将redisTemplate注入到ValueOperations，避免了ValueOperations<String, Object> valueOperations = redisTemplate.opsForValue(); 这样来获取ValueOperations；
 //redisTemplate并不是ValueOperations的实现类，这两个在继承上毫无联系的两个类是如何注入的呢。其实redisTemplate和ValueOperations一般不是父子关系或接口实现关系，其中的原理参考：https://blog.csdn.net/tiger0709/article/details/80745725
  /** String 字符串类型简单K-V操作*/
  ...
  /** List 针对list类型的数据操作*/
  @Resource(name = "redisTemplate")
  private ListOperations<String, String> valOpsList;
  ...
  public void saveSeq(String key, long seqNo) {
      //TODO 这里的redisTemplate和valOpsStr 区别是什么？
  ...
  /**
   * 保存到hash集合中 只在 key 指定的哈希集中不存在指定的字段时，设置字段的值。如果 key 指定的哈希集不存在，
   * 会创建一个新的哈希集并与 key 关联。如果字段已存在，该操作无效果。
   * @param hName 集合名
   * @param key
   * @param value
   */
  ...
  /**
   * 保存到hash集合中 只在 key 指定的哈希集中不存在指定的字段时，设置字段的值。
  ...
  //     */
  //    public <T> T get(String key, Class<T> clazz) throws JsonParseException,

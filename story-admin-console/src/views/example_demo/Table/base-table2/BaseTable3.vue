...
<script>
// 使用Popover弹出框实现点击弹出筛选项，进行筛选操作
//参考： https://blog.csdn.net/enjoylearning_zn/article/details/108026012
export default {
  name: 'BaseTable2',
  data () {
    return {
  ...
  methods: {
    handlerFilterFunc (type) {
      ...
      if (type === 'filter') {
        ...
        this.checkList = []
        this.colums = JSON.parse(JSON.stringify(Object.keys(this.tableData)))
      }
    }
  },
  created () {
    this.checkLabels = JSON.parse(JSON.stringify(Object.keys(this.tableData)))
    this.colums = JSON.parse(JSON.stringify(Object.keys(this.tableData)))
  }
}

<template>
  <div>
    <el-form ref="recordForm" :model="userTemp" :rules="rules" label-position="right" label-width="90px" style="margin:0 20px">
      ...
      <el-form-item label="邮箱" prop="phone" autocomplete="off">
...
<script>
// import { updateUserInfo } from '@/api/sysmgr/user'
export default {
  name: 'AccountInfo',
  data() {
    var _this = this
    ...
    return {
      ...
      rules: {
        ...
        email: [{ min: 7, max: 11, message: '长度为7-11', trigger: 'blur' }]
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    initUserInfo() {
      ...
      this.userTemp.account = 'admin'
      // this.userTemp.phone = this.$store.getters.phone
      this.userTemp.email = '1171205514@qq.com'
    },
    show() {

<template>
  <div class="app-container lunarFullCalendar">
    ...
    <el-dialog :visible.sync="modifyVisible" title="任务详情">
      <el-form
        ref="taskForm"
        :model="taskForm"
        :rules="rules"
      ...
      >
        ...
        <el-form-item label="标题" prop="title">
          <el-input v-model="taskForm.title" placeholder="请输入标题..." style="width:400px"/>
        </el-form-item>
        ...
        <el-form-item label="链接" prop="url">
          <el-input v-model="taskForm.url" placeholder="请输入链接..." style="width:400px"/>
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button v-if="taskForm.id" type="danger" @click="dropEvent">删除</el-button>
    ...
    eventClick(event, jsEvent, view) {
      ...
      findById(params).then(res => {
        if (res.result) {
          ...
          this.taskDate = [
            new Date(Date.parse(res.data.start.replace(/-/g, '/'))),
            new Date(Date.parse(res.data.end.replace(/-/g, '/')))
          ];
        } else {
